...
namespace Rookie.Ecom.Admin.Validators
{
    public class BrandDtoValidator : BaseValidator<BrandDto>
    {
        public BrandDtoValidator(IBrandService brandService)
        {
            ...
            RuleFor(m => m.Decription)
               .MaximumLength(ValidationRules.BrandRules.MaxLenghCharactersForDecription)
               .WithMessage(string.Format(ErrorTypes.Common.MaxLengthError, ValidationRules.BrandRules.MaxLenghCharactersForDecription))
               .When(m => !string.IsNullOrWhiteSpace(m.Decription));
